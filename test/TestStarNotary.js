...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.symbol.call(), "ST <^>");
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user = accounts[1];
    let starId = 10;
    await instance.createStar('awesome star 10', starId, {from: user});
    // 2. Call your method lookUptokenIdToStarInfo
    await instance.lookUptokenIdToStarInfo(starId);
    // 3. Verify if you Star name is the same
    assert.equal(await instance.lookUptokenIdToStarInfo(starId), 'awesome star 10');
});